...
const stacks = [
    {
        ...
        skills: [
            { name: "TypeScript", level: "Advanced", experience: "4+ years", descriptionKey: "TypeScript", color: "from-blue-500 to-blue-600" },
            { name: "Next.js", level: "Advanced", experience: "2+ years", descriptionKey: "Nextjs", color: "from-gray-700 to-gray-800" },
            { name: "MongoDB", level: "Advanced", experience: "2+ years", descriptionKey: "MongoDB", color: "from-green-500 to-green-600" },
            { name: "Express.js", level: "Advanced", experience: "3+ years", descriptionKey: "Expressjs", color: "from-yellow-600 to-orange-500" },
            { name: "Tailwind CSS", level: "Advanced", experience: "2+ years", descriptionKey: "Tailwind CSS", color: "from-cyan-500 to-cyan-600" },
            { name: "NGINX", level: "Intermediate", experience: "2+ years", descriptionKey: "NGINX", color: "from-green-600 to-green-700" },
            { name: "Python", level: "Intermediate", experience: "1+ years", descriptionKey: "Python", color: "from-yellow-500 to-yellow-600" },
            { name: "Java", level: "Intermediate", experience: "3+ years", descriptionKey: "Java", color: "from-orange-500 to-red-500" },
        ],
    },
    {
                            ...
                            <div className={`grid ${group.title === "additionalExperience" ? "grid-cols-7 gap-4" : "sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6"}`}>
                                {group.skills.map((skill, spos) => (
                                    <div key={skill.name} className={`group relative flex flex-col justify-between p-4 sm:p-5 rounded-xl bg-white/80 dark:bg-white/5 backdrop-blur-sm border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 transition-all duration-500 hover:scale-105 cursor-pointer ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: `${gpos * 200 + 400 + spos * 100}ms` }} onMouseEnter={() => setHover(skill.name)} onMouseLeave={() => setHover(null)}>
                                        ...
                                        {"experience" in skill && (
                                            <div className="relative mt-4 text-xs text-gray-500 dark:text-gray-400">{text("experience")}: {skill.experience}</div>
                                        )}
                                    </div>
                                ))}
                            </div>
