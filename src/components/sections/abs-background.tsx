...
export function CreativeBg({ particlesEnabled: on }: Props) {
        ...
        const loop = () => {
            setPts(p =>
                p.map(pt => {
                    let x = pt.x + pt.dx
                    let y = pt.y + pt.dy
                    x = x > innerWidth ? 0 : x < 0 ? innerWidth : x
                    y = y > innerHeight ? 0 : y < 0 ? innerHeight : y
                    return { ...pt, x, y }
                })
            )
    ...
    return (
        <div className="fixed inset-0 overflow-hidden pointer-events-none">
            ...
            {on && (
                <>
                    ...
                    <div className="absolute inset-0">
                        {gms.map(g => (
                            <div key={g.id} className="absolute border-2 border-purple-500/20 dark:border-purple-400/20" style={{ left: g.x, top: g.y, width: g.s, height: g.s, transform: `rotate(${g.rot}deg)`, opacity: g.o, borderRadius: g.t === "circle" ? "50%" : g.t === "triangle" ? "0" : "8px", clipPath: g.t === "triangle" ? "polygon(50% 0%, 0% 100%, 100% 100%)" : "none" }}/>
                        ))}
                    </div>

                    <div className="absolute w-64 h-64 rounded-full pointer-events-none transition-all duration-1000 ease-out" style={{ left: pos.x - 128, top: pos.y - 128, background: "radial-gradient(circle, rgba(139, 92, 246, 0.1) 0%, transparent 70%)" }}/>
            ...
            <div className="absolute inset-0 opacity-[0.02] dark:opacity-[0.05]" style={{ backgroundImage: `radial-gradient(circle, rgba(139, 92, 246, 0.5) 1px, transparent 1px)`, backgroundSize: "30px 30px" }}/>

            {["top-0", "bottom-0"].map((posY, i) => (
                <div key={`h-${i}`} className={`absolute ${posY} left-0 w-full h-px bg-gradient-to-r from-transparent via-${i % 2 === 0 ? "purple" : "blue"}-500/20 to-transparent ${ on ? `animate-pulse ${i ? "delay-500" : ""}` : "opacity-50" }`} />
            ))}

            {["left-0", "right-0"].map((posX, i) => (
                <div key={`v-${i}`} className={`absolute top-0 ${posX} w-px h-full bg-gradient-to-b from-transparent via-${i % 2 === 0 ? "purple" : "blue"}-500/20 to-transparent ${on ? `animate-pulse ${i ? "delay-1500" : "delay-1000"}` : "opacity-50"}`} />
            ))}
        </div>
    )
}